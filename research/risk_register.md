...
| Reduction kernels may be bandwidth/launch bound | High | Phase 3 reduced the native median from 141.0779 ms to 5.3505 ms, but it remains 5.8953× fused AdamW on the 73-tensor suite; profiler shows 11 launches per tensor step | Phase 3 remains `REVISE`; a future version needs a materially lower-launch multi-tensor execution without changing the map |
